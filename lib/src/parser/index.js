...
class Parser {
  ...
  parseExpression(scope, besides = null) {
    ...
    const state = this.tokenizer.createState();
    if (
      besides !== "NumberLiteral" &&
      (express = this.parseNumberLiteral(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "StringLiteral" &&
      (express = this.parseStringLiteral(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "BooleanLiteral" &&
      (express = this.parseBooleanLiteral(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "BinaryExpression" &&
      (express = this.parseBinaryExpression(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "ReferenceExpression" &&
      (express = this.parseReferenceExpression(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "ModifierExpression" &&
      (express = this.parseModifierExpression(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "IdentifierExpression" &&
      (express = this.parseIdentifierExpression(scope))
    )
      return express;
    state.resume();
    if (
      besides !== "ParameterExpression" &&
      (express = this.parseParameterExpression(scope))
    )
      return express;
    state.resume();
    return null;
  ...
  parseVariableDeclaration(scope) {
    ...
    if (!(0, checkers_js_1.isBuiltinType)(type)) return null;
    const mutableTok = this.tokenizer.getToken();
    let mutable = false;
    if (mutableTok.token === token_js_1.Token.Question) mutable = true;
    let name = mutable ? this.tokenizer.getToken() : mutableTok;
    if (!(0, checkers_js_1.isIdentifier)(name)) return null;
    ...
    if (!value) {
      ...
      new error_js_1.TokenMismatchError(
    ...
    const node = new VariableDeclaration_js_1.VariableDeclaration(
      value,
      new Identifier_js_1.Identifier(name.text, name.range),
      new TypeExpression_js_1.TypeExpression([type.text], false, type.range),
      mutable,
      Range_js_1.Range.from(value.range, this.tokenizer.position.toRange()),
    );
  ...
  parseFunctionDeclaration(scope) {
    ...
    if (!(0, checkers_js_1.isIdentifier)(fn) || fn.text !== "fn") return null;
    ...
    while (true) {
      ...
      if (tok !== token_js_1.Token.Comma) break;
    }
    if (this.tokenizer.getToken().token !== token_js_1.Token.Sub) return null;
    if (this.tokenizer.getToken().token !== token_js_1.Token.GreaterThan)
      return null;
    const returnType = this.tokenizer.getToken();
    if (!(0, checkers_js_1.isBuiltinType)(returnType)) return null;
    const block = this.parseBlockExpression(blockScope);
    if (!block) return null;
    const node = new Function_js_1.FunctionDeclaration(
      new Identifier_js_1.Identifier(name.text, name.range),
      params,
      new TypeExpression_js_1.TypeExpression(
        [returnType.text],
        false,
        returnType.range,
      ),
      block,
      new Scope_js_1.Scope(scope),
      exported,
      Range_js_1.Range.from(
        (exp === null || exp === void 0 ? void 0 : exp.range) || fn.range,
        this.tokenizer.position.toRange(),
      ),
    );
    if (scope.parentScope) {
      if (exported) {
        new error_js_1.SyntaxError(
          this.program,
          "Warn",
          "Exported functions must occur at the global scope! Not exporting function and moving on.",
          0x1,
          fn.range,
          "WARN",
        );
  ...
  parseEnumDeclaration(scope) {
    ...
    if (elementName.token !== token_js_1.Token.Identifier) return null;
    ...
    while (true) {
      ...
      if (trailing.text === "}" || trailing.text === ",") {
        const element = new EnumElement_js_1.EnumElement(
          new Identifier_js_1.Identifier(elementName.text, elementName.range),
          elementValue ||
            new NumberLiteral_js_1.NumberLiteral(
              index.toString(),
              this.tokenizer.position.toRange(),
            ),
          Range_js_1.Range.from(
    ...
    const node = new EnumDeclaration_js_1.EnumDeclaration(
      new Identifier_js_1.Identifier(name.text, name.range),
      elements,
      Range_js_1.Range.from(elementName.range, trailing.range),
    );
    this.program.globalScope.add(name.text, node);
    this.program.topLevelStatements.push(node);
    return node;
  ...
  parseIfStatement(scope) {
    ...
    if (!block) return null;
    const node = new IfStatement_js_1.IfStatement(
      condition,
      block,
      Range_js_1.Range.from(start, block.range),
    );
    return node;
  }
  parseModifierExpression(scope) {
    ...
    if (
      (hashToken === null || hashToken === void 0 ? void 0 : hashToken.text) !==
        "#" ||
      (openingBracketToken === null || openingBracketToken === void 0
        ? void 0
        : openingBracketToken.text) !== "[" ||
      (!(0, checkers_js_1.isIdentifier)(tagToken) &&
        tagToken.text !== "extern") ||
      (closingBracketToken === null || closingBracketToken === void 0
        ? void 0
    ...
    if (colonToken.token !== token_js_1.Token.Colon) {
      const node = new ModifierExpression_js_1.ModifierExpression(
        new Identifier_js_1.Identifier(tagToken.text, tagToken.range),
        null,
        Range_js_1.Range.from(hashToken.range, colonToken.range),
      );
      return node;
    }
    this.tokenizer.getToken();
    ...
    const contentId = new Identifier_js_1.Identifier(
    ...
    const node = new ModifierExpression_js_1.ModifierExpression(
      new Identifier_js_1.Identifier(tagToken.text, tagToken.range),
      contentId,
      Range_js_1.Range.from(hashToken.range, contentId.range),
    );
    return node;
  }
  parseFunctionImport(scope) {
    ...
    while (true) {
      ...
      params.push(param);
      const tok = this.tokenizer.getToken().token;
      if (tok === token_js_1.Token.RightParen) break;
      if (tok !== token_js_1.Token.Comma) break;
    }
    if (this.tokenizer.getToken().token !== token_js_1.Token.Sub) return null;
    if (this.tokenizer.getToken().token !== token_js_1.Token.GreaterThan)
    ...
    const returnType = this.tokenizer.getToken();
    if (!(0, checkers_js_1.isBuiltinType)(returnType)) return null;
    const node = new FunctionImport_js_1.FunctionImport(
      contentId,
      new Identifier_js_1.Identifier(name.text, name.range),
      params,
      new TypeExpression_js_1.TypeExpression(
      ...
      exported,
      Range_js_1.Range.from(hashToken.range, returnType.range),
    );
    scope.add(name.text, node);
    if (scope.parentScope)
      throw new Error(
        "Expected to find import function at top level, but found it elsewhere!",
      );
    this.program.topLevelStatements.push(node);
    return node;
  }
  parseBranchStatement(scope) {
    ...
    const start = this.tokenizer.position.toRange();
    const name = this.tokenizer.getToken();
    if (name.token !== token_js_1.Token.Identifier) return null;
    const block = this.parseBlockExpression(scope);
    if (!block) return null;
    const node = new BranchStatement_js_1.BranchStatement(
      new Identifier_js_1.Identifier(name.text, name.range),
      block,
      Range_js_1.Range.from(start, block),
    );
    scope.add(name.text, node);
    return node;
  }
  parseBranchToStatement(scope) {
    ...
    if (branchTo.token !== token_js_1.Token.Identifier) return null;
    const node = new BranchToStatement_js_1.BranchToStatement(
      new Identifier_js_1.Identifier(branchTo.text, branchTo.range),
      Range_js_1.Range.from(start, branchTo.range),
    );
    return node;
  }
  parseCallExpression(scope) {
    ...
    while (true) {
      ...
      if (tok.token === token_js_1.Token.RightParen) break;
      if (tok.token !== token_js_1.Token.Comma) return null;
    }
    if (!(0, checkers_js_1.isIdentifier)(calling)) return null;
    if (leftParen.token !== token_js_1.Token.LeftParen) return null;
    const node = new CallExpression_js_1.CallExpression(
      new Identifier_js_1.Identifier(calling.text, calling.range),
      args,
      Range_js_1.Range.from(calling.range, args[args.length - 1].range),
    );
    return node;
  }
  parseReferenceExpression(scope) {
    ...
    return new ReferenceExpression_js_1.ReferenceExpression(
      id.text,
      scope.get(id.text),
      id.range,
    );
  ...
  parseReturnStatement(scope) {
    ...
    if (!(0, checkers_js_1.isIdentifier)(rt) || rt.text !== "rt") return null;
    const expr = this.parseExpression(scope);
    if (!expr) return null;
    const node = new ReturnStatement_js_1.ReturnStatement(
      expr,
      Range_js_1.Range.from(rt.range, expr.range),
    );
    return node;
  }
  parseParameterExpression(scope) {
    const name = this.tokenizer.getToken();
    if (
      !(0, checkers_js_1.isIdentifier)(name) ||
      this.tokenizer.getToken().text !== ":"
    )
      return null;
    const type = this.tokenizer.getToken();
    if (!(0, checkers_js_1.isBuiltinType)(type)) return null;
    const node = new ParameterExpression_js_1.ParameterExpression(
      new Identifier_js_1.Identifier(name.text, name.range),
      new TypeExpression_js_1.TypeExpression([type.text], false, type.range),
      Range_js_1.Range.from(name.range, type.range),
    );
    scope.add(name.text, node);
    return node;
  }
  parseBlockExpression(scope) {
    ...
    if (token.token !== token_js_1.Token.LeftBracket) return null;
    const stmts = [];
    while (true) {
    ...
    if (this.tokenizer.getToken().token !== token_js_1.Token.RightBracket)
      return null;
    const node = new BlockExpression_js_1.BlockExpression(
      stmts,
      scope,
      Range_js_1.Range.from(token.range, this.tokenizer.position.toRange()),
    );
    return node;
  }
  parseBinaryExpression(scope) {
    ...
    if (left instanceof Identifier_js_1.Identifier) {
      if (scope.has(left.data)) {
        left = new ReferenceExpression_js_1.ReferenceExpression(
          left.data,
          left,
          left.range,
        );
    ...
    const node = new BinaryExpression_js_1.BinaryExpression(
      left,
      op,
      right,
      Range_js_1.Range.from(left.range, right.range),
    );
    // Check scope
    return node;
  }
  parseParenthesizedExpression(scope) {
    ...
    if (this.tokenizer.getToken().text !== ")") return null;
    if (!expr) return null;
    const node = new PathenthesizedExpression_js_1.ParenthesizedExpression(
      expr,
      Range_js_1.Range.from(start, expr.range),
    );
  }
  parseIdentifierExpression(scope) {
    const id = this.tokenizer.getToken();
    if (!(0, checkers_js_1.isIdentifier)(id)) return null;
